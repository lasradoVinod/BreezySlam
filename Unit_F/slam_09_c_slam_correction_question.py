...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        G3 = self.dg_dstate(self.state[0:3], control, self.robot_width)
        left, right = control
        ...
        V = self.dg_dcontrol(self.state[0:3], control, self.robot_width)
        R3 = dot(V, dot(control_covariance, V.T))

        #Using Matrix as AB
        #                CD
        B = zeros((3,2 * self.number_of_landmarks))
        C = zeros((2*self.number_of_landmarks ,3))
        D = eye(2 * self.number_of_landmarks)
        G = zeros ((3 + 2 * self.number_of_landmarks,3 + 2 * self.number_of_landmarks)) 
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        old_dim = (index) * 2 + 3
        state = zeros((dim))
        state[0:3 + (2 * self.number_of_landmarks)] = self.state
        state[3+2*index] = initial_coords[0]
        state[3+2*index+1] = initial_coords[1]

        self.state = state

        cov = zeros((dim,dim))
        cov[0:old_dim,0:old_dim] = self.covariance
        cov[old_dim,old_dim] = 1e6
        cov[old_dim+1,old_dim+1] = 10e6

        self.covariance = cov
        
        self.number_of_landmarks = index + 1
        return index
    ...
    def correct(self, measurement, landmark_index):
        ...
        H3 = self.dh_dstate(self.state[0:3], landmark, self.scanner_displacement)

        H = zeros((2,3 + 2*self.number_of_landmarks)) 
        H[:,0:3] = H3  # Replace this.

        H[:,3+2*landmark_index:3+2*(landmark_index+1)] = -1 * H[:,0:2]
