...
class ParticleFilter:
    ...
    def predict(self, control):
        ...
        sigma_l = (self.control_motion_factor * left) ** 2
        sigma_l = sigma_l + ((self.control_turn_factor * (left-right)) ** 2)
        sigma_l = sqrt (sigma_l)

        sigma_r = (self.control_motion_factor * right) ** 2
        sigma_r = sigma_r + ((self.control_turn_factor * (left-right)) ** 2)
        sigma_r = sqrt (sigma_r)
        
        new_particles = []
        for particle in self.particles:
            new_particles.append(self.g(particle,(
                    random.gauss(left,sigma_l),random.gauss(right,sigma_r)),self.robot_width))
    ...
    def probability_of_measurement(self, measurement, predicted_measurement):
        ...
        r_diff = predicted_measurement[0] - measurement[0]
        angle_diff = (predicted_measurement[1] - measurement[1] + pi) % (2 * pi) - pi

        return (normal_dist.pdf(r_diff,0.,self.measurement_distance_stddev) * normal_dist.pdf(angle_diff,0.,self.measurement_angle_stddev))
    ...
    def resample(self, weights):
        ...
        index = int(random.random() * len(weights)) 

        k = 0
        move = 0.
        new_particles = []        
        while k < len(weights):
            move = move + random.random() * 2 * max_weight
            while (move > weights[index]):
                move = move - weights[index]
    ...
    def get_mean(self):
        ...
        for particle in self.particles:
            mean_x = mean_x + particle[0]
            mean_y = mean_y + particle[1]
            mean_ac = mean_ac + cos(particle[2])
            mean_as = mean_as + sin(particle[2])

        M = len(self.particles)
        return (mean_x/M, mean_y/M, atan2(mean_as/M,mean_ac/M))  # Replace this.
